...
class BSDDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.task == 0:
            label_path = os.path.join(self.label_dir, img_name.replace('.jpg', '.mat'))
            label = self.load_mat_mask(label_path, field='Boundaries')
            support_label_path = os.path.join(self.label_dir, support_name.replace('.jpg', '.mat'))
            support_mask = self.load_mat_mask(support_label_path, field='Boundaries')
    ...
    def load_mat_mask(self, mat_path, field='Boundaries'):
        mat = sio.loadmat(mat_path)
        # Typically, the segmentation is under key 'groundTruth' or similar
        try:
            mask_struct = mat['groundTruth'][0, 0][field][0, 0]  # common format
        except Exception as e:
            raise ValueError(f"Unexpected .mat structure in {mat_path}: {e}")
        
        mask_struct[mask_struct == 0] = 255
        mask_struct[mask_struct == 1] = 0
